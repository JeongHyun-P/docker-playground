...
sudo systemctl enable nginx # 자동 재시작 설정

sudo rm /etc/nginx/sites-enabled/default # nignx 기본 설정 제거 (설정 충돌방지)
```
...
cd /home/ubuntu/app
mkdir -p env nginx scripts logs

# 환경변수 작성
vi env/front.env
vi env/back_blue.env
vi env/back_green.env # 설정 같으면 그냥 cp env/back.blue.env env/back.green.env
```
...
services:
    back_blue:
        image: 990738782497.dkr.ecr.ap-northeast-2.amazonaws.com/back
        container_name: back_blue
        env_file:
            - ../../env/back_blue.env
        volumes:
            - ../../logs/back_blue:/app/logs
...
upstream back_upstream {
    server back_blue:4001;
    # server back_green:4001;
}
...
2. 도커 이미지 빌드
   ...
   `docker build --platform linux/amd64 -t back .`

3. 빌드 완료 후 이미지에 태그 지정
   `docker tag back:latest 11111111111.dkr.ecr.ap-northeast-2.amazonaws.com/back:latest`

4. ECR에 이미지 푸시
   `docker push 11111111111.dkr.ecr.ap-northeast-2.amazonaws.com/back:latest`
...
# blue 최초 배포
docker compose -f compose/backend/blue.yml up -d
...
6. nginx/api.conf에 HTTPS 서버블록 추가
...
# 현재 활성 색상 판단
# ------------------------------------------
if grep -Eq "^[[:space:]]*server back_blue" "$NGINX_CONF"; then
  CURRENT="blue"
  NEXT="green"
elif grep -Eq "^[[:space:]]*server back_green" "$NGINX_CONF"; then
  CURRENT="green"
  NEXT="blue"
else
...
# ------------------------------------------
COMPOSE_FILE="$APP_DIR/compose/backend/$NEXT.yml"
...
sed -E "s/^[[:space:]]*server back_$CURRENT/# server back_$CURRENT/" "$NGINX_CONF" | \
sed -E "s/^[[:space:]]*#\s*server back_$NEXT/server back_$NEXT/" > "$TEMP_CONF"
...
# 이전 컨테이너 Down
# ------------------------------------------
echo "▶ stop old container: back_$CURRENT"
docker compose -p "$PROJECT_NAME" -f "$APP_DIR/compose/backend/$CURRENT.yml" down
